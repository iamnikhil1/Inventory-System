...
$sql_check = "SELECT MAX(id) AS max_id FROM items";
$result_check = mysqli_query($conn, $sql_check);
$row = mysqli_fetch_assoc($result_check);

if ($row['max_id']) {
    $next_id = $row['max_id'] + 1;
    $sql_reset = "ALTER TABLE items AUTO_INCREMENT = {$next_id}";
} else {
    $sql_reset = "ALTER TABLE items AUTO_INCREMENT = 1";
}
...
header("Location: http://localhost/Log/items.php");
// After deleting the lead
header("Location: items.php?message=Lead deleted successfully");
exit();
