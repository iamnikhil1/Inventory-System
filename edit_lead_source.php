...
<div class="container">
  ...
  <?php
       ...
       $id=$_GET['id'];
       $query = "SELECT * FROM source_lead WHERE id ={$id}";
       $result = mysqli_query($conn, $query) or die("Query Unsuccessful");

       if (mysqli_num_rows($result) > 0) {
        while ($rows = mysqli_fetch_assoc($result)) {
    ?>
    <form id="leadSourceForm" action="update_lead_source.php" method="POST">
      <h2>Update Lead Source</h2>
      ...
      <select id="status" name="status" class="form-control" value="<?php echo $rows['status']; ?>" required>
