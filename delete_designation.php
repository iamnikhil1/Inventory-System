...
// Delete lead
$id = $_GET['id'];
$delete_query = "DELETE FROM designation WHERE id = $id";
mysqli_query($conn, $delete_query) or die("Error deleting lead: " . mysqli_error($conn));

// Reset AUTO_INCREMENT
$sql_check = "SELECT MAX(id) AS max_id FROM designation";
$result_check = mysqli_query($conn, $sql_check);
$row = mysqli_fetch_assoc($result_check);

if ($row['max_id']) {
    $next_id = $row['max_id'] + 1;
    $sql_reset = "ALTER TABLE designation AUTO_INCREMENT = {$next_id}";
} else {
    $sql_reset = "ALTER TABLE designation AUTO_INCREMENT = 1";
}
...
header("Location: designation.php?message=Lead deleted successfully");
exit();
